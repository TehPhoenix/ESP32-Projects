...
const int kDoorOpenTimeBeforeNotify = 300000; // Length of time door to be allowed open befor notification sent.  5 minutes (Milliseconds)
...
// Door activation request on Blynk app
BLYNK_WRITE(V0) {
  if (param.asInt() == 1) {
    ActivateDoor();
    NewButtonPress = true;
...
void MonitorDoor() {
  if (InitialisationComplete != HIGH) { // Initialisation cycle not executed
    InitialiseDoor();
    InitialisationComplete = HIGH;
  }
...
void DetermineDoorStatus(int CurrentDoorStatus) {
  ...
  // If the door is not closed send a warning notification at the configured intival
  if (DoorStatusCode != 2) {  // Door is not closed
    if (DoorOpenedTime == 0) {
      DoorOpenedTime = millis();
    } else {
      if (millis() - DoorOpenedTime > kDoorOpenTimeBeforeNotify) {
        Blynk.notify("The Garage Door Is Still Open!");
        DoorOpenedTime = 0;
      }
    }
  } else {
